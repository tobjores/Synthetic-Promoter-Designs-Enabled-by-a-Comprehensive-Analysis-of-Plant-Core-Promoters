...
\renewcommand{\textTau}{{\symbolfont\symbol{61556}}}
...
\newenvironment{edfig}{%
	\begin{figure}%
		\stepcounter{edfig}%
		\setcounter{subfig}{0}%
		\pdfbookmark{\figurename\ \theedfig}{ext_data_figure\theedfig}
		\tikzsetnextfilename{ext_data_fig\theedfig}%
		\fignormal%
		\centering%
}{%		
%		\begin{tikzpicture}[remember picture,overlay]
%			\node[anchor=south east, shift={(-2.5cm,1.5cm)}] at (current page.south east) {\normalsize Jores \textit{et al.}, Ext. Data. Fig. \theedfig};
%		\end{tikzpicture}
	\end{figure}%
	\clearpage
}
...
\begin{document}
	...
	\ifmain
		...
		\begin{fig}
			\input{Figure2}%
			\caption{%
				\textbf{Plant core promoters span a wide range of activity.}\titleend
				\subfigref{A}\subfigref{B} Violin plots of the strength of plant promoters from the indicated species as measured by STARR-seq in tobacco leaves \parensubfig{A} or maize protoplasts \parensubfig{B} for libraries without ($-$) or with ($+$) the 35S enhancer upstream of the promoter. \nextentry
				\subfigref{C} Enrichment of selected GO terms for genes associated with the 1000 strongest promoters in the Arabidopsis (At), maize (Zm), and sorghum (Sb) promoter libraries without enhancer in tobacco leaves (top panel) and maize protoplasts (bottom panel). The red line marks the significance threshold (adjusted $p\ \textsf{value} \leq 0.05$). Non-significant bars are shown in gray. The $p$ values were determined using the gprofiler2 library in R with gSCS correction for multiple testing. Exact $p$ values are listed in Supplementary Table 11.\nextentry
				\subfigref{D}\subfigref{E} Violinplots of promoter strength (libraries without 35S enhancer) in tobacco leaves \parensubfig{D} or maize protoplasts \parensubfig{E}. Promoters were grouped by gene type.\nextentry
				In \parensubfig[\subfigref{A}\subfigref{B}\subfigref{D}]{E}, violin plots represent the kernel density distribution and the boxplots within represent the median (center line), upper and lower quartiles (box limits), and 1.5 $\times$ the interquartile range (whiskers) for all corresponding promoters. Numbers at the bottom of the plot indicate the number of tested promoters. Significant differences between two samples were determined using the two-sided Wilcoxon rank-sum test and are indicated: \issignif, $p \leq \signiflevelone$; \issignif\issignif, $p \leq \signifleveltwo$; \issignif\issignif\issignif, $p \leq \signiflevelthree$; \notsignif, not significant. Exact $p$ values are listed in Supplementary Table 11.
			}%
			\label{fig:strength}%
		\end{fig}
		\clearpage
		...
		\begin{fig}
			...
			\caption{%
				...
				\subfigref{B} Violin plots, boxplots and significance levels (as defined in \autoref{fig:strength}) of promoter strength for libraries without enhancer in tobacco leaves. Promoters are grouped by GC content to yield groups of approximately similar size.\nextentry
		...
		\begin{fig}
			...
			\caption{%
				...
				Violin plots, boxplots and significance levels in \parensubfigrange{A}{F} are as defined in \autoref{fig:strength}.
			}%
			\label{fig:enhEffect}
		\end{fig}
		...
		\begin{fig}
			...
			\caption{%
				...
				\subfigref{B} Violin plots of light-dependency (promoter strength\textsuperscript{light} divided by promoter strength\textsuperscript{dark}) for promoters in the libraries with ($+$) or without ($-$) the 35S enhancer.\nextentry
				\subfigref{C} Enrichment of selected GO terms for genes associated with the 1000 most light-dependent promoters. The red line marks the significance threshold (adjusted $p\ \textsf{value} \leq 0.05$). Non-significant bars are gray. The $p$ values were determined using the gprofiler2 library in R with gSCS correction for multiple testing. Exact $p$ values are listed in Supplementary Table 11. \nextentry
				...
				Violin plots, boxplots and significance levels in \parensubfigrange[\subfigref{B}]{D}{F} are as defined in \autoref{fig:strength}. Only one half is shown for violin plots in \parensubfigrange{D}{F}.
			}%
			\label{fig:lightDep}
		\end{fig}
		...
		
		{
			...
			\begin{edfig}
				...
				\caption{%
					...
					\subfigref{A} Correlation (Pearson's $r$) between the promoter strength and expression levels of the corresponding genes in the indicated species. Each boxplot (center line, median; box limits, upper and lower quartiles; whiskers, 1.5 $\times$ interquartile range; points, outliers) represents the correlation for all individual tissue samples in the RNA-seq dataset (see Methods). The number of samples in the RNA-seq dataset is indicated at the bottom of the plot.\nextentry
			...
			\begin{edfig}
				...
				\caption{%
					...
					\subfigref{B} Violin plots, boxplots and significance levels (as defined in \autoref{fig:strength}) of promoter strength for maize promoters without enhancer in the indicated assay system. Promoters without a TATA-box ($-$) were compared to those with a TATA-box outside ($+$/$-$) or within one of the three peaks highlighted in \parensubfig{A}.
				}%
				\label{edfig:TATAmaize}
			\end{edfig}
			...
			\begin{edfig}
				...
				\caption{%
					...
					\subfigref{H} Boxplots and significance levels (as defined in \autoref{fig:TATA}) for the relative strength of the promoter variants shown in (\textbf{\plainsubfigref{E}}-\textbf{\plainsubfigref{G}}). The corresponding WT promoter was set to 0 (horizontal black line).
				}%
				\label{edfig:BRE}%
			\end{edfig}
			...
			\begin{edfig}
				...
				\caption{%
					...
					\subfigrange{D}{I} Violin plots, boxplots and significance levels (as defined in \autoref{fig:strength}) of promoter strength for libraries without enhancer in tobacco leaves (\textbf{\plainsubfigref{D}}-\textbf{\plainsubfigref{F}}) or maize protoplasts (\textbf{\plainsubfigref{G}}-\textbf{\plainsubfigref{I}}). Promoters were grouped by the position of their TCP \parensubfig[\subfigref{D}]{G}, HSF \parensubfig[\subfigref{E}]{H} or NAC \parensubfig[\subfigref{F}]{I} transcription factor binding site relative to the TATA-box: either upstream (up) or downstream (down).
				}%
				\label{edfig:TFpos}%
			\end{edfig}
			...
			\begin{edfig}
				...
				\caption{%
					...
					\subfigref{D} Boxplots and significance levels (as defined in \autoref{fig:TATA}) for the relative light-dependency of promoters harboring mutations in the indicated transcription factor binding site as shown in (\textbf{\plainsubfigref{A}}-\textbf{\plainsubfigref{C}}). The corresponding wild type promoter was set to 0 (horizontal black line).
				}%
				\label{edfig:ligDepMutTFs}
			\end{edfig}
	...
	\ifsupp
		...
		\begin{sfig}
			...
			\caption{%
				...
				\subfigrange{A}{D} Violin plots of promoter strength \parensubfig[\subfigref{A}]{B} or GC content \parensubfig[\subfigref{C}]{D} in tobacco leaves \parensubfig[\subfigref{A}]{C} or maize protoplasts \parensubfig[\subfigref{B}]{D}. Promoters were grouped by GC content and split into promoters without (left half, darker color) or with (right half, lighter color) a TATA-box. Violin plots, boxplots and significance levels are as defined in \autoref{fig:strength}. Only one half is shown for violin plots.
			}%
			\label{sfig:TATA-GC}%
		\end{sfig}
		...
		\begin{sfig}
			...
			\caption{%
				...
				\subfigref{A}\subfigref{B} Violin plots, boxplots and significance levels (as defined in \autoref{fig:strength}) of enhancer responsiveness in tobacco leaves \parensubfig{A} or maize protoplasts \parensubfig{B}. Promoters associated with miRNA or protein-coding genes are compared.
			}%
