	...
	\ifmain
		...
		\begin{figure}[h]
			\caption{%
				...
				\subfigref{B} Each promoter library (At, Arabiopsis; Zm, maize; Sb, sorghum) contained two internal control constructs driven by the 35S minimal promoter without ($-$) or with ($+$) an upstream 35S enhancer. The enrichment ($\log_2$) of recovered mRNA barcodes compared to DNA input was calculated with the enrichment of the enhancer-less control set to 0. In all following figures this metric is indicated as promoter strength. Each boxplot (center line, median; box limits, upper and lower quartiles; whiskers, 1.5 $\times$ interquartile range; points, outliers) represents the enrichment of all barcodes linked to the corresponding \hl{internal control construct}.\nextentry
				\subfigref{C}\subfigref{D} Correlation \hl{(Pearson's $R^2$ and Spearman's $\rho$)} of two biological replicates of STARR-seq using the maize promoter libraries in tobacco leaves \parensubfig{C} or in maize protoplasts \parensubfig{D}.\nextentry
				\subfigref{E} Comparison of the strength of maize promoters in tobacco leaves and maize protoplasts. \hl{Pearson's $R^2$ and Spearman's $\rho$ are indicated.}\nextentry
				\subfigref{F}\subfigref{G} Violin plots of \hl{the strength of plant promoters from the indicated species} as measured by STARR-seq in tobacco leaves \parensubfig{F} or maize protoplasts \parensubfig{G} for libraries without ($-$) or with ($+$) the 35S enhancer upstream of the promoter. \nextentry
		...
		\begin{fig}
			...
			\caption{%
				...
				\subfigref{B} Violin plots (as defined in \autoref{fig:overview}) of light-dependency (promoter strength\textsuperscript{light} \hl{divided by} promoter strength\textsuperscript{dark}) for promoters in the libraries with ($+$) or without ($-$) the 35S enhancer.\nextentry
	...
	\ifsupp
		...
		\begin{sfig}
			...
			\caption{%
				...
				\hl{Pearson's $R^2$ and Spearman's $\rho$ are indicated in all plots.}
			}%
		...
		\begin{sfig}
			\input{SuppFig3}%
			\caption{%
				\textbf{Promoter strength and \textit{in vivo} expression levels are not correlated.}\titleend
		...
		\begin{sfig}
			...
			\caption{%
				...
				\subfigrange{A}{C} Histograms showing the number of promoters with a TCP \parensubfig{A}, HSF \parensubfig{B} or \hl{NAC} \parensubfig{C} transcription factor binding site at the indicated position.\nextentry
				\subfigrange{D}{I} Violin plots (as defined in \autoref{fig:overview}) of promoter strength for libraries without enhancer in tobacco leaves (\textbf{\plainsubfigref{D}}-\textbf{\plainsubfigref{F}}) or maize protoplasts (\textbf{\plainsubfigref{G}}-\textbf{\plainsubfigref{I}}). Promoters were grouped by whether their TCP \parensubfig[\subfigref{D}]{G}, HSF \parensubfig[\subfigref{E}]{H} or \hl{NAC} \parensubfig[\subfigref{F}]{I} transcription factor binding site is located upstream (up) or downstream (down) of the TATA-box.
			}%
			\label{sfig:TFpos}%
		\end{sfig}
		...
		\end{sfig}
			...
			\begin{sfig}
				\input{SuppFig15}%
				\caption{%
					\textbf{Mutations in transcription factor binding sites influence light-dependency.}\titleend
					\subfigrange{A}{C} One or two T>G mutations were introduced in binding sites for TCP \parensubfig[\subfigref{A}]{B} or WRKY \parensubfig{C} transcription factors. Which bases were mutated depended on the orientation in which the binding site was present in the wild type promoter.\nextentry
		...
		\begin{sfig}
			\input{SuppFig18}%
			\caption{%
				\textbf{The \textit{in silico} evolution is most effective in early rounds.}\titleend
